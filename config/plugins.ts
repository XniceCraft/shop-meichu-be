export default ({ env }) => ({
    "strapi-cache": {
        ...
        config: {
            ...
            ttl: 1000 * 60 * 10, // Time to live for cache items (10 minutes)
            size: 1024 * 1024 * 1024, // Maximum size of the cache (1 GB) (only for memory cache)
            allowStale: false, // Allow stale cache items (only for memory cache)
            cacheableRoutes: ["/api/dashboard-summary"], // Caches routes which start with these paths (if empty array, all '/api' routes are cached)
